...
class MRIScansPipeline():
    ...
    def extract_brain_part(self, input_image_path, output_filename):
        """Performs skull stripping using nipype.interfaces.fsl.BET.
        ...
        try:
            # Instantiate the BET interface
            bet_interface = fsl.BET()
            bet_interface.inputs.in_file = input_image_path
            bet_interface.inputs.out_file = full_output_path
            bet_interface.inputs.robust = True  # Corresponds to the -R option
            bet_interface.inputs.output_type = 'NIFTI_GZ' # Ensure gzipped NIfTI output
            # Other options like -f (fractional intensity threshold) or -g (gradient) can be set here if needed:
            # bet_interface.inputs.frac = 0.5 

            result = bet_interface.run() # Execute BET

            # Nipype usually creates the output file at the specified out_file path.
            # We double-check its existence.
            if not os.path.exists(full_output_path):
                # Check if Nipype put it in a working directory (less common if out_file is absolute)
                # For debugging, one might check result.outputs if the file isn't where expected.
                # logger.error(f"Nipype BET result outputs: {result.outputs}")
                error_message = f"BET output file not found at {full_output_path} after running Nipype fsl.BET."
                raise FileNotFoundError(error_message)
            
            return full_output_path
        except Exception as e:
            # Catch any exception from Nipype or file check
            error_message = f"BET (using Nipype fsl.BET) failed for input {input_image_path}!\nError: {str(e)}" 
